...
namespace DapperHelper.Core.Models
{
  ...
  public class UserCommand
  {
    ...
    public UserCommand(string commandText, object userObjectParameter)
    {
      ...
      {
        if (property.PropertyType.FullName != "System.String" && property.PropertyType.GetInterface("System.Collections.IEnumerable") != null)
        {
          throw new Exception($"{property.PropertyType.FullName} for enumeration types, it is necessary to use the SetParam method to set parameters");
        }
        userParameters.Add(new UserParameter(property.PropertyType, property.Name, property.GetValue(userObjectParameter)));
      }
    ...
    public void SetParameter<T>(string name, T value, object defaultValue = null)
    {
      ...
      else
      {
        ...
        if (data.IsEnumerable == true)
        {
          ...
          {
            throw new Exception($"The current SQL parameter :{data.Name} is an enumerated value, and a defaultValue is required to prevent exceptions when the enumeration list is empty");
          }
